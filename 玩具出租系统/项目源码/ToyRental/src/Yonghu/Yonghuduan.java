...
public class Yonghuduan extends JFrame implements ActionListener, Public_ {
	...
	Object a[][];
	Object name[] = {"序号","玩具类型","玩具主","价格","颜色","是否被租用"};
...
//		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	...
	public void init()
	{
		label1 = new JLabel("玩具租赁信息浏览系统");
		buttonOfKe = new JButton("   可 租 用 玩 具   ");
		...
		buttonOfWo = new JButton("   我   租   的   玩   具   ");
		buttonOfWo.addActionListener(this);
		label2 = new JLabel("输入需要租用的玩具：");
...
//		
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(source == buttonOfKe)//点击可以租用玩具的按钮
		{
			int success = 0;//用来判断 有没有 没被租用的玩具，如果能返回 “否” 值的集，则赋值为 1
			 int i=0;
			 while(i<50)
			 {
				 a[i][0]=" ";
			 ...
			 try {
				stmt = con.createStatement();
				 String sql= "select * from toy_information where hire= '否';";//查询表里面 没被租用的玩具
				 rs = stmt.executeQuery(sql);
		...
		else if (source == buttonOfQyueding)//点击 租用的确定键
		{
			...
			{
				JOptionPane.showMessageDialog(null, "请输入租用玩具的编号！");
			}
			else//不为空
			{
				this.connDB();
				...
				try {
					stmt = con.createStatement();
					sql = "select * from toy_information  where number='"+field.getText()+"' and hire = '否'";//获取输入的 序号并且 没被租用的信息
					rs = stmt.executeQuery(sql);
				} catch (SQLException e2) {
					e2.printStackTrace();
				}
				try {
					if(rs.next())//判断是否存在 此玩具
					{
						int n = JOptionPane.showConfirmDialog(this, "确定租用此玩具信息？","确认对话框",JOptionPane.YES_NO_OPTION);
						if(n == JOptionPane.YES_OPTION)//确认框
						{	
							try
							{
								stmt = con.createStatement();
								String sql2 = "update  toy_information set hire = '是',username = '"+username+"' where number='"+field.getText()+"';";//把租用的赋值为 是
					...
					{
						JOptionPane.showMessageDialog(null, "无法租用此玩具！");
					}
		...
		else if(source == buttonOfWo)//点击我租的玩具信息
		{
			int success = 0;//用来判断 有没有 没自己租用的玩具
			 int i=0;
			 while(i<50)
			 {
				 a[i][0]=" ";
			 ...
			 try {
				stmt = con.createStatement();
				 String sql= "select * from toy_information where username= '"+username+"';";//查询表里面 自己已经租用的玩具
				 rs = stmt.executeQuery(sql);
				 while(rs.next())//把查询到的信息写入 table
				 {
					 String number = rs.getString("number");
			 ...
			 if(success == 0)
			 {
				 JOptionPane.showMessageDialog(null, "您还没有租用任何玩具！");
			 }
		}
