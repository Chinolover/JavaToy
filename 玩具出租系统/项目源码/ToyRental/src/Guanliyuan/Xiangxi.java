...
public class Xiangxi extends JFrame implements ActionListener, Public_ {
...
//		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	...
	public void setArea()//打开界面就把数据库中的详细信息写入到area中
	{
		this.connDB();
		try {
		...
		try {
			...
			{
				JOptionPane.showMessageDialog(null,"没有此编号的玩具的详细信息！");
				new Guanliyuan(true);
			}
	...
	public void init()	//前端，详细信息的按钮以及布局
	{
		JLabel label = new JLabel("玩具的详细信息：");
	...
	public void actionPerformed(ActionEvent e) {
		Object source = e.getSource();	//这部分是详细信息
		...
		else if(source == buttonOfTi)
		{
			if(setEnable == 1)
			{
				String areaString = area.getText();//读入详细信息文本框
				this.connDB();
				try 
				{
					stmt = con.createStatement();		
					String sqlString ="update toy_information set information='"+areaString+"'where number = '"+number+"';";
