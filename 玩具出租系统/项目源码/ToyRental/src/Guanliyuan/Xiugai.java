...
public class Xiugai extends JFrame implements ActionListener, Public_ {
...
//		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	...
	public void init()
	{
		...
		JLabel label2 = new JLabel("     玩 具 型  : ");
		JLabel label3 = new JLabel("     玩 具 主  : ");
	...
	public void setText()//此函数功能是 把数据库 需要修改的信息 都打印到界面的文本框中
	{
		...
		try {
			...
			//String sql = "select * from toy_information (toytype,toyower,price,color,hire);";
			String sql = "select * from toy_information where number = '"+number+"';";
			rs = stmt.executeQuery(sql);
			if(rs.next())
			{
				  toytype = rs.getString("toytype");
				  toyower = rs.getString("toyower");
		...
		field2.setText(toytype);
		field3.setText(toyower);
	...
	public void actionPerformed(ActionEvent e) {
		...
		this.connDB();
		 ...
		 toytype = field2.getText() ;
		 toyower = field3.getText() ;
		...
		{
			if(toytype.equals("")||toyower.equals("")||price.equals("")||color.equals("")||hire.equals(""))
			{
			...
			else
			{
				...
				int n  = JOptionPane.showConfirmDialog(this, "确定修改此玩具信息？","确认对话框",JOptionPane.YES_NO_OPTION);//确认文本框
				if(n == JOptionPane.YES_OPTION)
				{
					...
					try {
						...
						stmt = con.createStatement();
						String str =  "update toy_information set toytype='"+toytype+"',toyower='"+toyower+"',price='"+price+"',color='"+color+"',hire='"+hire+"' where number = '"+number+"';";
